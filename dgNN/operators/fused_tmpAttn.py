...
import fused_tmpAttn as fused_tmpAttn
...
def GATConvFuse_inference(attn_row,attn_col,row_ptr,col_ind,negative_slope,in_feat):
    return fused_tmpAttn.gat_inference(attn_row,attn_col,row_ptr,col_ind,negative_slope,in_feat)
...
class FusedGATFunction(torch.autograd.Function):
    ...
    def forward(ctx,attn_row,attn_col,row_ptr,col_ind,col_ptr,row_ind,permute,negative_slope,in_feat,attn_drop):
        ...
        out_feat,edge_max,edge_sum,edge_mask=fused_tmpAttn.gat_forward(attn_row,attn_col,row_ptr,col_ind,negative_slope,in_feat,attn_drop)
        # print(edge_mask)
    ...
    def backward(ctx,grad_out):
        ...
        grad_feat,grad_attn_row,grad_attn_col=fused_tmpAttn.gat_backward(
